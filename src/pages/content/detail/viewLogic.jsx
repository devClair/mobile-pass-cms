...
export const useViewLogic = (props) => {
  ...
  const get = async (params) => {
    try {
      let data = await apiObject.getCmsContent(
        {
          content_no,
        },
        loadingFunction
  ...
  const update = async (params) => {
    const { content_no, content_title, content_content } = params;
    console.log({ params });
    ...
    try {
      ...
      let responseUpdateCmsInquiry = await apiObject.updateCmsContent(
        {
          content_no,
          content_title,
          content_content,
